...
const SkillsSection: FC = () => {
  const skillCategories: SkillCategory[] = [
    ...
    },
    {
      ...
      skills: [
        ...
        { name: "CI/CD", level: "Competent", description: "GitHub Actions, automated deployment" },
    ...
    },
    {
      ...
      skills: [
        ...
        { name: "Competitive Programming", level: "Competent", description: "Top 0.8% in 20K+ competition" }
      ]
    }
  ]
  ...
  const getLevelColor = (level: string) => {
    switch (level) {
      ...
      case 'Competent': return 'bg-blue-500/20 text-blue-400 border-blue-500/30'
    ...
    <section id="skills" className="my-24 scroll-mt-24">
        ...
        {skillCategories.map((category, index) => (
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              <div className={`p-2 rounded-lg bg-white/80 shadow-sm ${getIconColor(category.color)}`}>
              ...
              {category.skills.map((skill, skillIndex) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-medium border ${getLevelColor(skill.level)}`}>
                      {skill.level}
                    </span>
                  </div>
                  {skill.description && (
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-xl font-bold text-slate-800">Notable Achievements</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center p-4 bg-white/70 rounded-lg shadow-sm">
            ...
            <div className="text-sm text-slate-600">LeetCode Global Ranking</div>
          </div>
          <div className="text-center p-4 bg-white/70 rounded-lg shadow-sm">
            <div className="text-2xl font-bold text-teal-600 mb-1">Top 0.8%</div>
            <div className="text-sm text-slate-600">20K+ Competition Placement</div>
          </div>
          <div className="text-center p-4 bg-white/70 rounded-lg shadow-sm">
